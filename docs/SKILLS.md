...
- **`constitution-injection-mechanics.md`** — Implementation mechanics for project-constitution injection in Dungeon Master delegations. Covers the bootstrap exception (when injection begins firing relative to file creation), mid-session amendment behavior (re-reading the file at each delegation), conditional/no-op behavior (silent skip when the file is absent), and the full injection placement ordering rules for Pathfinder, Bitsmith, and Ruinor delegation prompts. Loaded on-demand by DM; extracted from `dungeonmaster.md` to reduce always-on token cost. Describes DM's implementation specifically; other delegating root contexts (e.g., the `wayblade` skill) follow the same shared block format (see `constitution-block-format.md`) but may differ in worktree/placement mechanics.

- **`constitution-block-format.md`** — Single source of truth for the `## Project Constitution` injection block format (heading, verbatim-contents placeholder, and reminder line) wrapped around a resolved `.claude/constitution.md`. Consumed by any delegating root context that injects the constitution into constitution-bearing agent prompts — currently the Dungeon Master and the `wayblade` skill.

- **`templates/`** — Delegation template bodies extracted from the Dungeon Master agent to keep its always-on prefix lean. Six files, each holding verbatim prompt text that DM embeds when delegating to sub-agents: `intake-gate-templates.md` (Askmaw and Pathfinder handoff), `investigative-gate-templates.md` (Tracebloom, Pathfinder handoff, Bitsmith trivial-fix, and Premise Check), `revision-delegation.md` (Phase 2 REVISE loop), `do-singlecommand-bitsmith-delegation.md` (`--execute` single-command path), `do-multistep-bitsmith-delegation.md` (`--execute` multi-step path), and `save-report-bitsmith-delegation.md` (Advisory Workflow `--save-report` write-task delegation). DM loads each file on demand via a precondition-framed pointer sentence; no other agents currently consume these files.
...
- **`wayblade`** — Manually-invoked (`disable-model-invocation: true`), single-context delivery-contract skill: a lightweight counterpart to the Dungeon Master's multi-agent orchestration, letting one continuous Claude Code session own clarify → plan → implement → debug → validate → docs → commit/push → PR end-to-end. Delegates only to Ruinor for the mandatory review gate (waivable via `--self-review`) and to specialists on explicit request; does not reproduce DM's `.worktrees/` sidecar, plan-file lifecycle, resumability, or audit trail. Invoke with `/wayblade`.
